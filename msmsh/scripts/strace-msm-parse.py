...
def hexdump( chars, width=16 ):
  ...
  while chars:
    ...
    print "DATA: %s%s%s%s" % ( '%08x ' % offset, ' '.join( map( lambda c: "%02x" % ord(c), line ) + ['  ']*filler),
...
class FHLogger(FHBase):
  ...
  def read(self, data):
    ...
    if self.packets_w:
      ...
      print "RAW: dir=read fd=%i fn='%s' len=%i" % (self.id, self.filename, len(data))
      hexdump(data)
    if packet:
      print "PACKET: dir=read fd=%i fn='%s' len=%i" % (self.id, self.filename, len(data))
      hexdump(data)
  ...
  def write(self, data):
    ...
    if self.packets_w:
      ...
      print "RAW: dir=write fd=%i fn='%s' len=%i" % (self.id, self.filename, len(data))
      hexdump(data)
    if packet:
      print "PACKET: dir=write fd=%i fn='%s' len=%i" % (self.id, self.filename, len(data))
      hexdump(packet)
