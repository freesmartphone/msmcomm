...
class FHLogger(FHBase):
  ...
  def read(self, data):
    ...
    if self.packets_w:
      ...
      print "RAW: dir=read fd=%i fn='%s'" % (self.id, self.filename)
      hexdump(data)
      print "\n"
    if packet:
      print ""
      print "PACKET: dir=read fd=%i fn='%s'" % (self.id, self.filename)
      #self.identify(packet)
      self.process_frame_header(packet)
  ...
  def write(self, data):
    ...
    if self.packets_w:
      ...
      print "RAW: dir=write fd=%i fn='%s'" % (self.id, self.filename)
      hexdump(data)
      print "\n"
    if packet:
      print ""
      print "PACKET: dir=write fd=%i fn='%s'" % (self.id, self.filename)
      self.process_frame_header(packet)
      #self.identify(packet)
      #hexdump(packet)

  def process_frame_header(self, packet):
      if len(packet) < 3:
          return
      type = ord(packet[1]) >> 4
      seq = ord(packet[2]) >> 4
      ack = ord(packet[2]) & 0xf
      print "frame (type=%s, seq=%02x, ack=%02x)" % (frame_type_names[type-1], seq, ack)
      print "packet[1] = 0x%x packet[2] = 0x%x" % (ord(packet[1]), ord(packet[2]))
